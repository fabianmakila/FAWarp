...
public final class ConfigurationLoader<C> {
	...
	public ConfigurationLoader(Class<C> configClass, Path configPath, UnaryOperator<ConfigurationOptions> optionsModifier) {
		this.loader = YamlConfigurationLoader.builder().path(configPath).nodeStyle(NodeStyle.BLOCK).indent(2).defaultOptions(optionsModifier).build();
		try {
	...
	public ConfigurationLoader(Class<C> configClass, Path configPath) {
		this(configClass, configPath, UnaryOperator.identity());
	}
